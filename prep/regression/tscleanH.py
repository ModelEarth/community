# To create projection, change DATA_COMBO_PATH to combo_zcta_sm_projection.csv
# And change last=2016 to last=2022 (the furthest in data/raw)
...
def combine_files(file_paths):
    ...
    df = pd.concat([load_csv(p, year=True) for p in file_paths],sort=False)
    return clean_missing(df)
def combo_pipeline(verbose=True):
    """ this function is to get all data from raw folder and combine into 1 file """
    raw_paths = get_raw_paths()
    #print(raw_paths) //there are 10 years
